...
const D3D11_INPUT_ELEMENT_DESC CaseApp::VertexPosColor::inputLayout[3] = {
	...
	{"OFFSET",0,DXGI_FORMAT_R32G32B32_FLOAT,1,0,D3D11_INPUT_PER_INSTANCE_DATA,1},//指定每份实例数据绘制1份实例
};

CaseApp::CaseApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
	m_MainWndCaption = L"01_case_draw_instance";
}
...
bool CaseApp::initEffect()
{
	...
	std::string vsCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/instance_feature/01_case_draw_instance/01_case_draw_instance_vs.cso");
	std::string vsFileName = FileSystem::getPath("shader_source/01_sole_feature/instance_feature/01_case_draw_instance/01_case_draw_instance_vs.hlsl");
	...
	std::string psCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/instance_feature/01_case_draw_instance/01_case_draw_instance_ps.cso");
	std::string psFileName = FileSystem::getPath("shader_source/01_sole_feature/instance_feature/01_case_draw_instance/01_case_draw_instance_ps.hlsl");
...
bool CaseApp::initResource()
{
	...
	{
		{vec4(-0.8f,0.5f,0.2f,1.0f),vec4(1.0f,0.0f,0.0f,1.0f)},
		{vec4(-0.6f,-0.5f,0.2f,1.0f),vec4(1.0f,0.0f,0.0f,1.0f)},
		{vec4(-1.0f,-0.5f,0.2f,1.0f),vec4(1.0f,0.0f,0.0f,1.0f)}
	};
	...
	vbd.ByteWidth = sizeof(insData);
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	ZeroMemory(&initData, sizeof(initData));
	initData.pSysMem = insData;
	HR(m_pd3dDevice->CreateBuffer(&vbd, &initData, m_pVertexBuffer[1].GetAddressOf()));



	unsigned int stride[2] = { sizeof(VertexPosColor),sizeof(InstanceData)};
	unsigned int offset[2] = { 0, 0};

	ID3D11Buffer* buffers[2] = { m_pVertexBuffer[0].Get(),m_pVertexBuffer[1].Get() };

	m_pd3dImmediateContext->IASetVertexBuffers(0, 2, buffers,stride,offset);
	//设置图元类型，设置输入布局
...
void CaseApp::DrawScene()
{
	...
	m_pd3dImmediateContext->DrawInstanced(3,5,0, 0);
