...
CaseApp::CaseApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
	m_MainWndCaption = L"01_case_stream_output";
	m_EnableDebug = false;
}
...
bool CaseApp::initEffect()
{
	...
	std::string vsCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/stream_output_feature/01_case_stream_output/01_case_stream_output_vs.cso");
	std::string vsFileName = FileSystem::getPath("shader_source/01_sole_feature/stream_output_feature/01_case_stream_output/01_case_stream_output_vs.hlsl");
	...
	std::string psCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/stream_output_feature/01_case_stream_output/01_case_stream_output_ps.cso");
	std::string psFileName = FileSystem::getPath("shader_source/01_sole_feature/stream_output_feature/01_case_stream_output/01_case_stream_output_ps.hlsl");
	...
	std::string gsFileName = FileSystem::getPath("shader_source/01_sole_feature/stream_output_feature/01_case_stream_output/01_case_stream_output_gs.hlsl");
	std::wstringstream gsFileWss;
	gsFileWss << gsFileName.c_str();

	//创建流输出几何着色器
	HR(CreateShaderFromFile(nullptr, gsFileWss.str().c_str(), "GS", "gs_5_0", blob.ReleaseAndGetAddressOf()));

	HR(m_pd3dDevice->CreateGeometryShaderWithStreamOutput(blob->GetBufferPointer(), blob->GetBufferSize(),
		posColorLayout, ARRAYSIZE(posColorLayout), nullptr, 0, D3D11_SO_NO_RASTERIZED_STREAM, nullptr, m_pGeometryShader.GetAddressOf()));
