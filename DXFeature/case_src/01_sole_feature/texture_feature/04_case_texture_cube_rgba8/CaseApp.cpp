...
CaseApp::CaseApp(HINSTANCE hInstance)
	: D3DApp(hInstance)
{
	m_MainWndCaption = L"04_case_texture_cube_rgba8";
	m_EnableDebug = false;
...
bool CaseApp::initEffect()
{
	...
	std::string vsCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/texture_feature/04_case_texture_cube_rgba8/04_case_texture_cube_rgba8_vs.cso");
	std::string vsFileName = FileSystem::getPath("shader_source/01_sole_feature/texture_feature/04_case_texture_cube_rgba8/04_case_texture_cube_rgba8_vs.hlsl");
	...
	std::string psCsoFileName = FileSystem::getPath("shader_source/01_sole_feature/texture_feature/04_case_texture_cube_rgba8/04_case_texture_cube_rgba8_ps.cso");
	std::string psFileName = FileSystem::getPath("shader_source/01_sole_feature/texture_feature/04_case_texture_cube_rgba8/04_case_texture_cube_rgba8_ps.hlsl");
...
bool CaseApp::initResource()
{
	...
	for (j = 0; j < 6; j++)
	{
		m_pd3dImmediateContext->UpdateSubresource(m_pTexture2D.Get(),
													D3D10CalcSubresource(0,D3D11_TEXTURECUBE_FACE_POSITIVE_X+j,tex2dDesc.MipLevels),
	...
	srvDesc.TextureCube.MipLevels = 1;//使用1mipmap
	srvDesc.TextureCube.MostDetailedMip = 0;//初始使用1等级的map
...
void CaseApp::updateMat()
{
	...
	m_constMat.viewMat = DirectX::XMMatrixTranspose(DirectX::XMMatrixLookAtLH(DirectX::XMVectorSet(0.0f, 0.0f, -2.0f, 1.0f),
